# Scala Guide & Code Convention
Base on some open Scala guides, our experience, and preferences. It's not strong rules, it's only our point of view.
...
Sometimes absending of culture in development become the reason of crash. We try to create the rules to minimizy it, base on the open Scala guides, commonly used best practices, our expierence and preferences.
Taken for the base:

1. [Databricks Scala Guide](https://github.com/databricks/scala-style-guide)
2. [Scala Best Practices](https://github.com/alexandru/scala-best-practices)
3. [Official Style Guide](https://docs.scala-lang.org/style/)
## <a name='syntactic'>Syntactic Style</a>
* [Naming Convention](#naming)
* [Variable Naming Convention](#variable-naming)
* [Method Naming Convention](#method-naming-conventions)
* [Rule of 30](#rule_of_30)
* [Class length](#class_length)
...
- Constants should be all PascalCase and be put in a companion object.
  ```scala
...
- Variables should be named in camelCase style and should have self-evident names.
  ...
  val clientPort = ClientPort(value)
  ```
...
### <a name='class_length'>Class length</a>

If the length of a class consists of more than 500 lines, it must be refactored to different classes. If you have this case probably you also have a problem with your abstraction. Rethink your idea.

- A class should contain less than 500 lines of code.

### <a name='method-naming-conventions'>Method naming convention</a>

For methods, which return:
- list of elements ```Effect[Seq[X]]``` or ```Future[Seq[X]]``` - ```listElemnts(..)``` 

  ```scala
  def listApples(..): Effect[Seq[T]] = ???
  ```

- one element ```Effect[X]``` or ```Future[X]``` - ```getElement(..)```

  ```scala
  def getApple(..): Effect[T] = ???
  ```

- one optional element ```Effect[Option[X]``` or ```Future[Option[X]]``` - ```findElement(..)```

  ```scala
  def findApple(..): Effect[Option[T]] = ???
  ```
...
- For classes whose header doesn't fit in a single line, put the extension on the next line with 2 space indent, and add a blank line after the class header.
  ```scala
  class Foo(
...
- The excessive number of blank lines is discouraged.
...
Put curly braces even around one-line conditional or loop statements. The only exception is if you are using if/else as a one-line ternary operator that is also side-effect free.
```scala
// Correct:
if (true) {
  println("Wow!")
...
### <a name='api-conventions'>API conventions</a>
**Path-params namings for selection and search:**
* For sorting use next params: `orderBy` and `orderDir` (`asc`, `desc`, default - `asc`)
* For search by date range use `from` / `till`
* For simple string search use param `search`
* For specifying the offset of search results: `offset` (0 - first element)
* For specifying quantity of results: `limit`
